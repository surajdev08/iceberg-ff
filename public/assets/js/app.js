 ...
 * Safe for Client-Side Route Transitions & Page Re-mounts
 */

(function initIcebergWrapper() {
  window.__ICEBERG_CLEANUPS__ = window.__ICEBERG_CLEANUPS__ || [];

  window.bootIceberg = function() {
    // Run and clear all transient cleanups from previous page mount
    if (window.__ICEBERG_CLEANUPS__) {
      window.__ICEBERG_CLEANUPS__.forEach(cleanup => {
        try { cleanup(); } catch (e) {}
      });
      window.__ICEBERG_CLEANUPS__ = [];
    }

    // ==========================================================================
    // A. PERSISTENT BLOCK (Runs exactly once for persistent layout elements)
    // ==========================================================================
    if (!window.__ICEBERG_PERSISTENT_BOOTED__) {
      window.__ICEBERG_PERSISTENT_BOOTED__ = true;
      ...
      document.querySelectorAll('.nav-link').forEach(link => {
        link.addEventListener('click', () => {
          const activeNavLinks = document.querySelector('.nav-links');
          const activeMobileNavToggle = document.querySelector('.mobile-nav-toggle');
          if (activeNavLinks) {
            activeNavLinks.classList.remove('active');
          }
          if (activeMobileNavToggle) {
            const spans = activeMobileNavToggle.querySelectorAll('span');
            if (spans.length >= 3) {
      ...
      // Live Status Uptime & Dynamic Ping Widget (Footer)
      const statusIndicatorText = document.getElementById('footer-live-ping');
      if (statusIndicatorText) {
        const footerPingInterval = setInterval(() => {
          const activeIndicatorText = document.getElementById('footer-live-ping');
          if (activeIndicatorText) {
            const randomPing = Math.floor(Math.random() * 8) + 3; // Ping fluctuations between 3ms and 10ms
            activeIndicatorText.textContent = `${randomPing}ms`;
          }
        }, 3000);
        // Clean up even if footer is persistent, in case layout is hard reloaded/unmounted
        window.__ICEBERG_CLEANUPS__.push(() => clearInterval(footerPingInterval));
      }
    }

    // ==========================================================================
    // B. TRANSIENT BLOCK (Runs and re-initializes elements on every mount)
    // ==========================================================================

    // 1. Light / Dark Mockup Toggle
    ...
    // 2. Dynamic Canvas Charting Engine (Hero & Mobile Mockups)
    ...
    class ChartEngine {
      constructor(canvas, maxPoints = 50, isMobile = false) {
        ...
        // Handle resizing without leaking the event listener
        const resizeHandler = () => this.resize();
        window.addEventListener('resize', resizeHandler);
        window.__ICEBERG_CLEANUPS__.push(() => window.removeEventListener('resize', resizeHandler));
    ...
    if (orderBookContainer) {
      ...
      // Render order book once — kept static, no live refresh
      generateOrderBook();
    }

    // 4. Multi-Asset Live Ticker Engine
    const tickerItems = [
    ...
    if (priceTickerTrack) {
      priceTickerTrack.innerHTML = ''; // Clear previous tickers to prevent duplication
      ...
      const tickerInterval = setInterval(() => {
        ...
        if (cards.length === 0) return; // Guard against absent elements on other routes
        ...
        if (cardA && cardB) {
          ...
          if (valSpanA && valSpanB) {
            const currentVal = parseFloat(valSpanA.getAttribute('data-price'));
            if (isNaN(currentVal)) return;
            
            const changePct = (Math.random() - 0.5) * 0.0001; // tiny fluctuation
            ...
            setTimeout(() => {
              const checkA = document.getElementById(`price-card-${randomIdx}`);
              const checkB = document.getElementById(`price-card-${isDoubleOne}`);
              if (checkA) checkA.classList.remove('tick-up', 'tick-down');
              if (checkB) checkB.classList.remove('tick-up', 'tick-down');
            }, 500);
          }
        }
      }, 800);

      window.__ICEBERG_CLEANUPS__.push(() => clearInterval(tickerInterval));
    }

    // 5. Technology Stack Detail Switcher
    ...
    if (techCards.length > 0 && techDetailIcon && techDetailTitle) {
      techCards.forEach(card => {
        card.addEventListener('click', () => {
          // Switch active card styling safely
          ...
          if (db) {
            ...
            if (panel) {
              ...
              setTimeout(() => {
                const innerSvg = techDetailIcon.querySelector('svg');
                if (innerSvg) innerSvg.innerHTML = db.svg;
                
                if (techDetailTitle) techDetailTitle.textContent = db.title;
                if (techDetailSub) techDetailSub.textContent = db.sub;
                if (techDetailDesc) techDetailDesc.textContent = db.desc;
                
                if (metricLatency) metricLatency.textContent = db.metrics.latency;
                if (metricCapacity) metricCapacity.textContent = db.metrics.capacity;
                if (metricSecurity) metricSecurity.textContent = db.metrics.security;

                const activePanel = document.querySelector('.tech-detail-panel');
                if (activePanel) {
    ...
    // 6. Solutions Panel Carousel System
    ...
    if (solutionsButtons.length > 0 && solutionsCards.length > 0) {
      solutionsButtons.forEach(btn => {
        btn.addEventListener('click', () => {
          // Toggle active menu button safely
    ...
    if (contactForm && formFeedback) {
      contactForm.addEventListener('submit', (e) => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            const activeFormFeedback = document.getElementById('contact-form-feedback');
            if (activeFormFeedback) {
              activeFormFeedback.classList.remove('success');
              activeFormFeedback.textContent = '';
            }
          }, 6000);
        }, 1500);
      });
    ...
    // 8. Interactive Markets Sandbox Playground Engine
    ...
    if (sandboxAssetName && sandboxTerminalLog) {
      ...
      sandboxAssetTabs.forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          // Update UI safely
          const activeIconEl = document.querySelector('.sandbox-asset-icon');
          if (activeIconEl) activeIconEl.textContent = asset.icon;
          
          const activeAssetName = document.getElementById('sandbox-asset-name');
          const activeAssetCat = document.getElementById('sandbox-asset-cat');
          const activeSpreadVal = document.getElementById('sandbox-spread-val');
          
          if (activeAssetName) activeAssetName.textContent = asset.name;
          if (activeAssetCat) activeAssetCat.textContent = asset.category;
          if (activeSpreadVal) activeSpreadVal.textContent = asset.spread;

          // Set Initial Prices
          const sellRate = asset.basePrice;
          const buyRate = asset.basePrice + asset.spreadOffset;
          
          const activeSellRate = document.getElementById('sandbox-sell-rate');
          const activeBuyRate = document.getElementById('sandbox-buy-rate');
          
          if (activeSellRate) activeSellRate.textContent = sellRate.toFixed(asset.decimals);
          if (activeBuyRate) activeBuyRate.textContent = buyRate.toFixed(asset.decimals);

          // Append log line safely
          const activeTerminalLog = document.getElementById('sandbox-terminal-log');
          if (activeTerminalLog) {
            const logLine = document.createElement('div');
            logLine.className = 'terminal-log-line init';
            logLine.innerHTML = `&gt; [${formatTime()}] SWITCHED ASSET: Loaded ${asset.name} feeds.`;
            activeTerminalLog.appendChild(logLine);
            activeTerminalLog.scrollTop = activeTerminalLog.scrollHeight;
          }
        });
      });

      // 2. Continuous price fluctuation for active sandbox asset
      const sandboxInterval = setInterval(() => {
        const asset = sandboxAssets[activeSandboxKey];
        if (!asset) return;

        // Route Guard: only process if the sandbox playground elements are present on screen
        const activeAssetName = document.getElementById('sandbox-asset-name');
        ...
        if (activeSellRate) activeSellRate.textContent = sellPrice.toFixed(asset.decimals);
        if (activeBuyRate) activeBuyRate.textContent = buyPrice.toFixed(asset.decimals);

        // Trigger flash effect safely
        if (sellBox && buyBox) {
          ...
          setTimeout(() => {
            const checkSellBox = document.querySelector('.sandbox-price-box.sell');
            const checkBuyBox = document.querySelector('.sandbox-price-box.buy');
            if (checkSellBox) checkSellBox.classList.remove('tick-up', 'tick-down');
            if (checkBuyBox) checkBuyBox.classList.remove('tick-up', 'tick-down');
          }, 400);
        }
      }, 900);

      window.__ICEBERG_CLEANUPS__.push(() => clearInterval(sandboxInterval));
      ...
      const executeSimulatedTrade = (type) => {
        ...
        const activeVolumeSlider = document.getElementById('sandbox-volume-slider');
        const activeLeverageSlider = document.getElementById('sandbox-leverage-slider');
        const activeTerminalLog = document.getElementById('sandbox-terminal-log');
        const activeSellRate = document.getElementById('sandbox-sell-rate');
        const activeBuyRate = document.getElementById('sandbox-buy-rate');

        if (!activeVolumeSlider || !activeLeverageSlider || !activeTerminalLog || !activeSellRate || !activeBuyRate) return;

        const volume = parseFloat(activeVolumeSlider.value).toFixed(2);
        const leverage = activeLeverageSlider.value;
        const rate = type === 'BUY' ? parseFloat(activeBuyRate.textContent) : parseFloat(activeSellRate.textContent);
        ...
        activeTerminalLog.appendChild(line1);

        setTimeout(() => {
          const innerTerminalLog = document.getElementById('sandbox-terminal-log');
          if (innerTerminalLog) {
            ...
            innerTerminalLog.appendChild(line2);
            innerTerminalLog.scrollTop = innerTerminalLog.scrollHeight;
          }
        }, 250);
        ...
        setTimeout(() => {
          ...
          if (innerTerminalLog) {
            ...
            innerTerminalLog.appendChild(line3);
            innerTerminalLog.scrollTop = innerTerminalLog.scrollHeight;
          }
        }, 550);
      };
    ...
    // 9. Immersive HTML5 Canvas Snowfall Particle System (Ice Ocean Atmosphere)
    ...
    if (snowfallCanvas) {
      ...
      if (sCtx) {
        ...
        let sWidth = sParent ? sParent.offsetWidth : window.innerWidth;
        let sHeight = sParent ? sParent.offsetHeight : 600;

        snowfallCanvas.width = sWidth;
        snowfallCanvas.height = sHeight;

        const onSnowResize = () => {
          const activeCanvas = document.getElementById('snowfall-canvas');
          if (activeCanvas) {
            const activeParent = activeCanvas.parentElement;
            sWidth = activeParent ? activeParent.offsetWidth : window.innerWidth;
            sHeight = activeParent ? activeParent.offsetHeight : 600;
            activeCanvas.width = sWidth;
            activeCanvas.height = sHeight;
          }
        };

        window.addEventListener('resize', onSnowResize);
        window.__ICEBERG_CLEANUPS__.push(() => window.removeEventListener('resize', onSnowResize));
        ...
        const drawParticles = () => {
          const activeCanvas = document.getElementById('snowfall-canvas');
          if (!activeCanvas) return; // Terminate loop if canvas is unmounted
        ...
        const snowfallLoop = () => {
          const activeCanvas = document.getElementById('snowfall-canvas');
          if (!activeCanvas) return; // Exit loop if page transitions
          drawParticles();
          snowLoopId = requestAnimationFrame(snowfallLoop);
        };

        window.__ICEBERG_CLEANUPS__.push(() => cancelAnimationFrame(snowLoopId));
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', window.bootIceberg);
  } else {
    window.bootIceberg();
  }
})();
